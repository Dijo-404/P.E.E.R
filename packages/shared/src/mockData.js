...
import { generateIndianName, generateRollNumber, generateSchoolTimestamp, generateRealisticScore, generateCompletionPercentage, generatePhoneNumber, generateEmail, generateAttendancePercentage, generateStudyStreak, generatePoints, SeededRandom, } from './utils/generators';
 ...
 */
export function generateMockCourses(studentClass, studentId) {
 ...
 */
export function generateMockProgress(courses, studentId) {
    const rng = new SeededRandom(studentId.charCodeAt(0));
 ...
 */
export function generateMockBadges(totalPoints, currentStreak, seed) {
    const rng = new SeededRandom(seed);
 ...
 */
export function generateMockQuizQuestions(subject, count = 10) {
    const questions = [];
 ...
 */
export function generateMockCommunityPosts(students, count = 20) {
    const posts = [];
 ...
 */
export function generateMockAssignments(studentClass, count = 10) {
    const assignments = [];
    const subjects = SUBJECTS_BY_CLASS[studentClass] || SUBJECTS_BY_CLASS['6'];
...
export const defaultMockData = {
    ...
    get currentStudentBadges() {
        return generateMockBadges(this.currentStudent.totalPoints, this.currentStudent.currentStreak, 1);
    },
