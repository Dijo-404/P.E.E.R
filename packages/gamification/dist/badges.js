...
export async function initializeBadges() {
    ...
    for (const badge of BADGES) {
        ...
        if (!existing) {
            await db.run(`INSERT INTO badges (id, name, description, icon, criteria_type, criteria_threshold, criteria_subject, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                badge.id,
                badge.name,
                badge.description,
 ...
 */
export async function checkBadgeEligibility(userId, badgeId) {
    const db = await getDatabase();
    // Check if already earned
    const existing = await db.get('SELECT id FROM user_badges WHERE user_id = ? AND badge_id = ?', [userId, badgeId]);
    if (existing)
        return false;
    // Get badge criteria
    ...
    switch (badge.criteria.type) {
        ...
        case 'mastery':
            return await checkMasteryCriteria(userId, badge.criteria.threshold, badge.criteria.subject);
        case 'streak':
...
export async function awardBadge(userId, badgeId) {
    ...
    await db.run('INSERT INTO user_badges (id, user_id, badge_id, earned_at) VALUES (?, ?, ?, ?)', [id, userId, badgeId, earnedAt]);
    const badge = BADGES.find((b) => b.id === badgeId);
    if (!badge)
        return null;
    return {
...
export async function getUserBadges(userId) {
    const db = await getDatabase();
    const userBadges = await db.all(`SELECT b.*, ub.earned_at
     FROM badges b
 ...
 */
export async function checkAndAwardAllBadges(userId) {
    const newBadges = [];
    for (const badge of BADGES) {
...
async function checkPointsCriteria(userId, threshold) {
    const db = await getDatabase();
    const user = await db.get('SELECT points FROM users WHERE id = ?', [userId]);
    return (user?.points || 0) >= threshold;
}
async function checkMasteryCriteria(userId, threshold, subject) {
    const db = await getDatabase();
...
async function checkStreakCriteria(_userId, _threshold) {
    // Would use calculateDailyStreak from points-engine
    // For now, simplified check
    return false; // Placeholder
}
async function checkPeerHelpCriteria(userId, threshold) {
    const db = await getDatabase();
    const result = await db.get(`SELECT COUNT(*) as count
     FROM tutoring_sessions
     WHERE tutor_id = ? AND status = 'completed' AND rating >= 4`, [userId]);
    return (result?.count || 0) >= threshold;
}
async function checkCustomCriteria(userId, badgeId) {
    // Custom logic for specific badges
    if (badgeId === 'first_steps') {
        const db = await getDatabase();
        const result = await db.get('SELECT COUNT(*) as count FROM learning_progress WHERE user_id = ? AND status != "not_started"', [userId]);
