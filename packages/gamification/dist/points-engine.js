 ...
 */
export async function awardPoints(userId, reason, customAmount) {
 ...
 */
export async function calculateDailyStreak(userId) {
    const db = await getDatabase();
    // Get all unique dates when user accessed content
    const accessDates = await db.all(`SELECT DISTINCT DATE(last_accessed_at / 1000, 'unixepoch') as date
    ...
    const daysSinceLastAccess = Math.floor((new Date(today).getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24));
