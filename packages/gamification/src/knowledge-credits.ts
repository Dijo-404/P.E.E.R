...
export async function transferCredits(
    ...
    toUserId: string,
    amount: number,
    reason: KnowledgeCredit['reason'],
    metadata?: Record<string, any>
): Promise<KnowledgeCredit> {
    if (amount <= 0) {
    ...
    const sender = await db.get<{ knowledge_credits: number }>(
        'SELECT knowledge_credits FROM users WHERE id = ?',
        [fromUserId]
    );
    ...
    await db.run(
        `INSERT INTO knowledge_credits (id, from_user_id, to_user_id, amount, reason, metadata, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [id, fromUserId, toUserId, amount, reason, JSON.stringify(metadata || {}), createdAt]
    );
...
export async function awardCredits(
    ...
    await db.run(
        `INSERT INTO knowledge_credits (id, from_user_id, to_user_id, amount, reason, metadata, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [id, 'system', userId, amount, reason, JSON.stringify(metadata || {}), Date.now()]
    );
    ...
    // Get updated balance
    const user = await db.get<{ knowledge_credits: number }>(
        'SELECT knowledge_credits FROM users WHERE id = ?',
        [userId]
    );
...
export async function getTotalCreditsEarned(userId: string): Promise<number> {
    ...
    const result = await db.get<{ total: number }>(
        'SELECT SUM(amount) as total FROM knowledge_credits WHERE to_user_id = ?',
        [userId]
    );
...
export async function getTotalCreditsSpent(userId: string): Promise<number> {
    ...
    const result = await db.get<{ total: number }>(
        'SELECT SUM(amount) as total FROM knowledge_credits WHERE from_user_id = ?',
        [userId]
    );
